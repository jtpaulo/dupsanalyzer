...
//print file DB
DB **dbstats; // DB structure handle
DB_ENV **envpstats;
...
int gen_stats(DB **dbpor,DB_ENV **envpor){
	...
	while ((ret = cursorp->get(cursorp, &key, &data, DB_NEXT)) == 0) {
	   ...
	   //get hash from berkley db
	   uint64_t ndups = (unsigned long long int)((struct hash_value *)data.data)->cont;
	   uint64_t size = (unsigned long long int)((struct hash_value *)data.data)->size;
	   ndups=ndups-1;
...
{
	printf("Usage:\n");
	printf(" -m, -s and -o can be used together or individually if only one operation is intended\n");
	printf(" -m<db1 folder path> <db2 folder path>\t(Merge db1 into db2)\n");
	printf(" -s<db folder path>\t\t(Print duplicate statistics of db.)\n");
...
int main (int argc, char *argv[]){
	...
	//path of stats database folder
	char dbstatspath[100];
	int printstats=0;
	...
	char dbfolderbasepath[100];
  	...
  	while ((argc > 1) && (argv[1][0] == '-'))
  	{
		switch (argv[1][1])
		{
			...
			case 's':
			if(argc<2){
				printf("NUmber of arguments is lower than the necessary to use -s\n");
				usage();
				exit(0);
			}
			strcpy(dbstatspath,&argv[1][2]);
			++argv;
			--argc;
			printstats=1;
  	...
  	if(printstats==1){
  		...
  		dbstats=malloc(sizeof(DB *));
  		envpstats=malloc(sizeof(DB_ENV *));

  		init_db(dbstatspath,dbstats,envpstats);
		gen_stats(dbstats,envpstats);
  		close_db(dbstats,envpstats);
